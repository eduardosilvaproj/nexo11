...
function gerarPDFPremium(relatorio: RelatorioEstimativa) {
  ...
  const ambientesHtml = ambientes.map(a =>
    `<div class="amb-row"><div class="nome">${a.nome}</div><div class="preco"><div class="medio">${fmt(a.totalMedio)}</div><div class="range">${fmt(a.totalMin)} — ${fmt(a.totalMax)}</div></div></div>`
  ).join("");
...
<div class="invest-section"><div class="invest-label">Faixa de Investimento</div><div class="invest-grid"><div class="invest-card min"><div class="card-label">Mínimo</div><div class="card-valor">${fmt(relatorio.total_minimo * 1.4)}</div></div><div class="invest-card med"><div class="card-label">Médio</div><div class="card-valor">${fmt(relatorio.total_medio * 1.1)}</div></div><div class="invest-card max"><div class="card-label">Máximo</div><div class="card-valor">${fmt(relatorio.total_maximo * 1.2)}</div></div></div></div>
    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="flex items-start justify-between">
          ...
          <Button className="gap-2" onClick={() => gerarPDFPremium(relatorio)}>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-4 bg-gray-50 rounded-lg border">
            ...
            <p className="text-xl font-bold text-gray-700">{formatCurrency(relatorio.total_minimo * 1.4)}</p>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg border">
            <p className="text-xs text-gray-500 mb-1 uppercase tracking-wide">Médio</p>
            <p className="text-xl font-bold text-gray-700">{formatCurrency(relatorio.total_medio * 1.1)}</p>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg border">
            <p className="text-xs text-gray-500 mb-1 uppercase tracking-wide">Máximo</p>
            <p className="text-xl font-bold text-gray-700">{formatCurrency(relatorio.total_maximo * 1.2)}</p>
          </div>
        </div>
      </Card>
          ...
          {ambientes.map((amb) => (
            <div key={amb.nome} className="flex items-center justify-between p-4 rounded-lg bg-gray-50/80 border border-gray-100">
              ...
              <div className="text-right">
                <p className="font-bold text-gray-900">{formatCurrency(amb.totalMedio)}</p>
      ...
      <Card className="p-4 bg-stone-50 border-stone-200">
        <p className="text-xs text-stone-500">
          Estimativa preliminar. Valores podem variar conforme acabamento, ferragens e acess&oacute;rios.
          Solicite uma visita t&eacute;cnica para or&ccedil;amento definitivo.
        </p>
      </Card>
    </div>
  );
