...
export const RelatorioEstimativaView = ({ relatorio }: RelatorioEstimativaViewProps) => {
  ...
  const handleDownloadPDF = () => {
    ...
    console.log('Download PDF', relatorio);
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold">
            Móveis Identificados ({relatorio.moveis.length})
          </h3>
        </div>
            ...
            return (
              <div
                ...
                className={`p-4 border rounded-lg \${
                  hasAlertas ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200'
                }`}
              >
                  ...
                  {estimativa && (
                    <div className="text-right ml-4">
                      ...
                      <p className="font-semibold text-green-600">
                        {formatCurrency(estimativa.preco_medio)}
                      </p>
                      <p className="text-xs text-gray-500">
                        {formatCurrency(estimativa.preco_minimo)} -{' '}
                        {formatCurrency(estimativa.preco_maximo)}
                      </p>
                    </div>
                  )}
                </div>
                ...
                {hasAlertas && (
                  <div className="mt-3 pt-3 border-t border-yellow-200">
                    {movel.alertas!.map((alerta, aIdx) => (
                      <div key={aIdx} className="flex items-start gap-2 text-sm text-yellow-800">
      ...
      <Card className="p-4 bg-amber-50 border-amber-200">
        <p className="text-sm text-amber-800">
          <strong>Importante:</strong> Esta é uma estimativa preliminar baseada em análise
          automatizada. O orçamento final deve ser elaborado no Promob após medição técnica
          e validação de todos os detalhes do projeto.
        </p>
      </Card>
    </div>
  );
